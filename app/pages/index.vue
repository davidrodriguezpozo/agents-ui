...
import type { NowKind } from '~/utils/nowQueue'
 ...
 *
 * **It is the home page now, and it absorbed the Now queue.** Those were two
 * screens asking one question badly: Now ranked everything that needed you,
 * including every pull request — which this page already drew in full, with the
 * merge and review affordances a one-line row cannot carry. Two counts of the
 * same work, on two screens, free to disagree. The rest of Now was three kinds
 * of row nothing else surfaces at all: a session frozen mid-turn, a ritual that
 * has stopped working, and a decision nobody has said why about.
 *
 * So the queue is a band at the top of this page, minus the kinds below it —
 * see `queueOmit`, which is conditional rather than fixed because "Ready here"
 * is only drawn from two sessions upwards. `/land` is still an alias: the
 * shortcut, the palette entry and anybody's bookmark all still work.
 */

definePageMeta({
  // Every link, bookmark, `g l` and palette entry written before the merge.
  // Costs one line and means nothing anybody typed stops working.
  alias: ['/land'],
})
...
/**
 * Whether this machine has anything set up at all.
 *
 * The gate the welcome used to sit behind on Now, carried over unchanged: a
 * machine with no agents, commands, skills or plugins has not been configured,
 * and the honest first screen for it is the welcome rather than a landing page
 * reporting that nothing is waiting.
 */
const hasLibrary = computed(() =>
  agents.value.length > 0 || commands.value.length > 0
  || skills.value.length > 0 || plugins.value.length > 0)

onMounted(() => { void fetchPlugins(); void fetchSkills(); void loadSettings() })

/**
 * Where Claude Code keeps its configuration, and the one control that changes it.
 *
 * Carried over from Now rather than dropped with the rest of that page, because
 * this was the only place in the app it existed: `useClaudeDir().set` has no
 * other caller, so losing the field would have meant losing the ability to point
 * this at a second `~/.claude` at all. Folded, at the bottom, as it was.
 */
const dirInput = ref('')
const settingDir = ref(false)

onMounted(() => { dirInput.value = claudeDir.value || '' })
...
async function changeDir() {
  ...
  try {
    ...
    await Promise.all([
      fetchAgents(), fetchCommands(), fetchPlugins(), fetchSkills(), loadSettings(),
    ])
  } finally {
...
/**
 * The line that says the rest is fine.
 *
 * Landed is deliberately not on it any more. On Now it linked here; on this
 * page it would point at the page you are already reading, and a link to
 * yourself is a link somebody presses once.
 */
...
const otherwise = computed(() => Boolean(running.value || money.value))
...
/**
 * What the queue at the top must not rank, because this page draws it below.
 *
 * Pull requests always: every row the queue would draw as a `review` is a row
 * in one of the four bands under "On GitHub", with the verdict, the already-
 * started join and the merge button the queue's single line cannot hold.
 *
 * Sessions ready to land only *when the train is actually drawn*, which is from
 * two sessions upwards — see `showTrain`. One finished session gets no train,
 * and omitting its row unconditionally would be how the only piece of work that
 * is actually ready disappears off the front page.
 */
const queueOmit = computed<NowKind[]>(() =>
  showTrain.value && workingDir.value ? ['review', 'ready-session'] : ['review'])
...
<template>
  <div>
    ...
    <div class="page-container page-container--measure py-4 space-y-7">
      <!--
        What needs you, above what is merely ready.

        `inline` because the all-clear this draws on its own page — "Nothing is
        waiting on you" — is a claim about the whole morning, and printed above
        five bands of pull requests it is simply false. Here an empty queue says
        nothing at all, which is correct: the bands underneath are the page.
      -->
      <NowQueue v-if="hasLibrary" inline :omit="queueOmit" />
      ...
      <!--
        First run, and the one thing on this page that is not about work.

        It was Now's, and Now is this page — so an empty machine still gets the
        welcome rather than an empty landing screen explaining that no pull
        request is waiting, which on a machine with nothing configured is true
        and useless.
      -->
      <WelcomeOnboarding
        v-if="!hasLibrary"
      ...
      <!-- Advanced: where the configuration this reads actually lives -->
      <details>
        ...
        <div class="rounded-lg p-4 mt-2 bg-card">
          <p class="fs-sm mb-3 text-label">
            This is where Claude Code stores your agents, commands, and settings.
      ...
      <!-- One line, not a legend: the cheatsheet is the thing, this points at it -->
      <button
      ...
      <EmptyState
        v-if="nothingAnywhere && hasLibrary"
