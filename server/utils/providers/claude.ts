...
/**
 * Claude Code, behind the provider seam.
 *
 * This is a move rather than a rewrite, and the distinction is the point: the
 * permission broker, the steer channel, the sandbox host detection, the budget
 * and turn-limit handling and the `result` bookkeeping are the same code that
 * used to be the body of `executeRun`, so a Claude Code session's behaviour and
 * its records on disk are unchanged in every respect. If making the seam had
 * required editing a test, the seam would have been in the wrong place.
 *
 * What stayed behind in `executeRun` is only what is true of any provider:
 * setting the status, catching a failure, handing undelivered mid-turn messages
 * to the session's queue, and persisting.
 */
...
async function runTurn(turn: ProviderTurn): Promise<SteerMessage[]> {
   ...
   */
  const prompt = openSteerChannel(run.id, run.input, turn.images)
  ...
  try {
    for await (const message of query({
      ...
      options: {
        ...
        // of unrelated runs.
        ...toQueryOptions(options, turn.resumeSessionId, turn.maxBudgetUsd),
      ...
      if (message.type === 'system' && message.subtype === 'init') {
        patch({ sdkSessionId: message.session_id })
      }
          ...
          for (const block of content) {
            if ((block as { type?: string }).type === 'tool_result') {
              ...
              if (options.sandbox.enabled) {
                ...
                if (refused.length) {
                  patch({ refusedHosts: [...new Set([...(run.refusedHosts ?? []), ...refused])] })
                }
              }
      ...
      if (message.type === 'result') {
        ...
        if (stoppedEarly) {
          patch({
            needsAttention: true,
            stoppedBy: subtype === 'error_max_budget_usd' ? 'budget' : 'turns',
          })
        }
